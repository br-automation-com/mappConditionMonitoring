...
{REDUND_CONTEXT} {REDUND_UNREPLICABLE} FUNCTION_BLOCK fltRead 	(*receive data by using the flat stream mode*)
	VAR_INPUT
...
{REDUND_CONTEXT} {REDUND_UNREPLICABLE} FUNCTION_BLOCK fltWrite (*write data by using the flat stream mode*)
	VAR_INPUT
		enable 		: BOOL; 				 (*enables execution*)
		cfg 		: UDINT; 				 (*configures the FltGen interface*)
